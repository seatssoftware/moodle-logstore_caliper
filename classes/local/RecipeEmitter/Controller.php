...
class Controller extends \stdClass {
    ...
    public function create_event(array $translatorevent) {
        ...
        if (isset(static::$routes[$route])) {
            ...
            $arrayExtensions = array('user_email'=>$translatorevent['user_email'], 'user_external_id'=>$translatorevent['user_external_id']);

            if(!is_null($translatorevent['user_custom_fields']))
            {
                foreach ($translatorevent['user_custom_fields'] as $index => $userCustomField) {
                $arrayExtensions["{$index}"] = "{$userCustomField}";
                }
            }

            $person->setExtensions($arrayExtensions);
