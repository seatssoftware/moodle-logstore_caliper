...
class Repository extends PhpObj {
     ...
     * @return PhpObj
     */
    public function readQuizGrade($quiz, $userid) {
        return $this->readStoreRecord('quiz_grades', ['userid' => $userid, 'quiz' => $quiz]);
    }
     ...
     * @return PhpObj
     */
    public function readAssignGrade($assignment, $userid) {
        return $this->readStoreRecord('assign_grades', ['userid' => $userid, 'assignment' => $assignment]);
    }
     ...
     * @return PhpArr
     */
    public function readUserInfoData($userId) {
        $userInfoDataFields = $this->readStoreRecords('user_info_data', ['userid' => $userId]);
        $resultFields = [];
        foreach ($userInfoDataFields as $index => $userInfoDataField) {
            try {
                $userInfoField = $this->readStoreRecord('user_info_field', ['id' => $userInfoDataField->fieldid]);
                $fieldName = trim(strtolower(preg_replace('/\s+/', '_', $userInfoField->name)));
                $resultFields[$fieldName] = $userInfoDataField->data;
            }
            catch (\Exception $e) {
                // Question not found; maybe it was deleted since the event. 
                // Don't add the question to the list, but also don't block the attempt event.
